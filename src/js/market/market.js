...
// Импортируем шаблоны
import { createNavItem, createProductList, createProductPage, createCartPage, createCartBlock } from "./tamplates";
...
(function (factory) {
    ...
    const market = (_window.market = function (globalSettingJson, productListJson, userCallbacks, settings) {
        ...
        const oneStepBack = () => {
            const menuItems = marketMenuWrap.querySelectorAll(`.market-header__nav-item`);
        ...
        const stateAfterSearch = (id, name) => {
            ...
            const lastMenuItem = marketMenuWrap.querySelector(`.market-header__nav li:last-child`);
        ...
        const createCurrentStateData = () => {
            ...
            const nameOfItem = productItem.querySelector(`.market-products__product-title`).textContent;
        ...
        const onProductHendler = () => {
            const productListWrap = marketContentWrap.querySelector(`.market-products__list`);
            const previousBtn = marketContentWrap.querySelector(`.market-btn--prev-step`);
        ...
        const checkForNeedCarousel = () => {
            // Находим все товары
            const productsItem = document.querySelectorAll(`.market-products__product`);
        ...
        const changeClassOfActiveOption = () => {
            ...
            const optionList = marketContentWrap.querySelector(`.market-product__option-list`);
            ...
            if (
                ...
                const optionBtn = optionList.querySelectorAll(`.market-product__option-item--active`);
                // Удаляем активный класс у всех опций
                optionBtn.forEach(item => item.classList.remove(`market-product__option-item--active`));
        ...
        const addProductToCart = (product, productPrice, optionName, optionValue) => {
        ...
        const onProductPageHandler = product => {
            ...
            if (productWrap !== null) {
                // Обёртка опций
                const optionWrap = productWrap.querySelector(`.market-product__option-wrap`);
                ...
                if (event.target.classList.contains(`market-btn--add-to-cart`)) {
                    ...
                    if (optionWrap !== null) {
                        ...
                        const optionName = optionWrap.querySelector(`.market-product__option-title`).textContent;
        ...
        const changePrice = (productData, element) => {
            ...
            const productTotalPrice = element.querySelector(`.market-cart__product-total-price`);
            // Изменяем общую цену внутри карточки товара
            productTotalPrice.textContent = (productData.price * productData.quantity).toLocaleString(`ru-RU`);

            // Находим обёртку количества товара
            const priceQuantity = element.querySelector(`.market-cart__price-quantity`);
        ...
        const changeQuantityProduct = (productData, element) => {
            ...
            const quantityInput = element.querySelector(`.market-cart__quantity-input`);
        ...
        const deleteProductFromCart = (productIndex, element) => {
            ...
            const deleteProductBtn = element.querySelector(`.market-cart__delete-wrap`);
        ...
        const changeProductDataInCart = () => {
            const currentCartElement = event.target.closest(`.market-cart__product-content`);
            ...
            if (currentCartElement !== null) {
                ...
                const checkProduct = item => {
                    ...
                    const productTitle = currentCartElement.querySelector(`.market-cart__title`).textContent;
                    // Находим опцию текущего товара
                    const productOption = currentCartElement.querySelector(`.market-cart__option`);
                    // Если опции есть
                    return productOption !== null
                        ? // элемент должен совпадать по имени и названию опции
                          item.name === productTitle && item.option.optionValue === productOption.textContent
                ...
                const currentCartItemIndex = cart.findIndex(product => checkProduct(product));
        ...
        const sendOrder = parentElement => {
            ...
            if (event.target === btnOrder) {
                const productTextarea = parentElement.querySelector(`.market-cart__textarea`);

                if (productTextarea !== null) {
                    productTextarea.remove();
                }

                const productItem = parentElement.querySelectorAll(`.market-cart__product-content`);
                const orderArray = [];
                ...
                for (let product of productItem) {
                    ...
                    const quantityOfProduct = product.querySelector(`.market-cart__price-quantity`);
                    const priceOfProduct = product.querySelector(`.market-cart__product-total-price`);
                ...
                const totalPrice = parentElement.querySelector(`.market-cart__total-price`);
        ...
        const onBackToMainPageBtn = parentElement => {
            ...
            const toMainFromCartBtn = parentElement.querySelector(`.market-cart__link-to-main`);
        ...
        searchInput.oninput = debounce(() => {
            ...
            const lastNavItem = marketMenuWrap.querySelector(`.market-header__nav li:last-child`);
        ...
        marketMenuWrap.addEventListener(`click`, () => {
            ...
            if (event.target.tagName === `LI`) {
                ...
                // Находим индекс элемента
                const indexNavItem = [...navItems].findIndex(item => item.textContent === textMenuItem);
