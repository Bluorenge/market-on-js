...
export default class OptionListComponent extends AbstractComponent {
    ...
    setAddOptionBtnHandler(handler) {
        this.getElement().querySelector(`.data-maker__add--option-item`).addEventListener(`click`, handler);
    }
    ...
    setInputHandler(handler) {
        this.getElement().querySelector(`.data-maker__input--option-list-name`).oninput = debounce(handler, 300);
    }
    ...
    getOptionListName() {
        ...
        // })()
        const nameOptionList = this.getElement().querySelector(`.data-maker__input--option-list-name`).value;
