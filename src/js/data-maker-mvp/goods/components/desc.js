...
export default class DescComponent extends AbstractComponent {
    ...
    setInputHandler(handler) {
        this.getElement().querySelector(`.data-maker__input--desc-product`).oninput = debounce(handler, 300);
    }

    setRemoveDescHandler(handler) {
        this.getElement().querySelector(`.data-maker__delete-btn--desc`).addEventListener(`click`, handler);
    }
}
