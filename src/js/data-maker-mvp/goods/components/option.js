...
export default class OptionComponent extends AbstractComponent {
    ...
    addErrorClass() {
        this.getElement().querySelector(`.data-maker__input--option-name`).classList.add("data-maker__input--error");
        this.getElement().querySelector(`.data-maker__input-error-message`).classList.remove("visually-hidden");
    }

    removeErrorClass() {
        this.getElement().querySelector(`.data-maker__input--option-name`).classList.remove("data-maker__input--error");
        this.getElement().querySelector(`.data-maker__input-error-message`).classList.add("visually-hidden");
    }

    setInputHandler(handler) {
        this.getElement().querySelector(`.data-maker__input--option-name`).oninput = debounce(handler, 300);
        this.getElement().querySelector(`.data-maker__input--option-price`).onchange = debounce(handler, 300);
        this.getElement().querySelector(`.data-maker__input--option-active`).onchange = debounce(handler, 300);
    }

    setRemoveOptionBtn() {
        this.getElement().querySelector(`.data-maker__delete-btn--option`).classList.add(`visually-hidden`);
    }

    setRemoveOptionBtnHandler(handler) {
        this.getElement().querySelector(`.data-maker__delete-btn--option`).addEventListener(`click`, handler);
    }
