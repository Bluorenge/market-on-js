...
const listItemTemplate = (globalSetting, option, listItem) => {
    ...
    const oneClickBtn = isOneClick
        ...
        : ``;
    const oneClickClass = isOneClick ? ` market-products__product-bottom--one-click` : "";

    const bottomContentTemplate = () => {
        return `<div class="market-products__product-bottom${oneClickClass}">
    <span class="market-products__product-price">${createPrice(listItem)} ${globalSetting.currency}</span>
    ...
    const isActive = isProduct && !listItem.active ? ` style="display: none;"` : ``;
